...
' ------------------------------------------------------------------------------
	Public Sub LoadData(paramID)
		...
		'-- Get menu item names
		Set DBConn = Server.CreateObject("ADODB.Recordset")
		SQLString = "GetModuleMenuData_V2 @idModule = " & p_id_module
		DBConn.Open SQLString, oCNDB, 3, 3
		if NOT DBConn.EOF then
		...
		While NOT DBConn.EOF
            ...
            %>
     		<div class="AreaArt_pic6_<%=p_counter%>">
                <a href="/article.asp?id=<%= articleItem.IDArticle %>">
					<img src="../<%= Replace(articleItem.MainThumbPictureFull, "_thumb", "_small") %>" width="100%" height="87" alt="" />
				</a>
            </div>
			<div class="AreaArt_pic6_<%=p_counter%>_title">
                <a style="font-size: 12px; font-family: Helvetica, Arial;" href="/article.asp?id=<%= articleItem.IDArticle %>">
                    <h3 class="aad-title"><%= articleItem.Title %></h3>
				</a>
            </div>	            
        <%
